#!/usr/bin/env python3
"""BBQ Experiment Analysis for Qwen-32B
...
# Baselines - filter for qwen_32B only
cot = load_baseline_combined(load_batch_experiment, prefix='qwen_32B')
nocot = load_batch_experiment('qwen_32B_force_immediate')

# Blank space experiments (static)
blank_static = {}
blank_static_values = [5, 10, 50, 100, 500]
for n in blank_static_values:
    exp = load_batch_experiment(f'qwen_32B_blank_static_{n}_')
    if exp:
        blank_static[n] = exp
...
blank_median = {}
for mult in [1, 2, 5]:
    exp = load_batch_experiment(f'qwen_32B_blank_dynamic_median_{mult}x_')
    if exp:
...
blank_max = {}
for mult in [1, 2, 5]:
    exp = load_batch_experiment(f'qwen_32B_blank_dynamic_max_{mult}x_')
    if exp:
        blank_max[mult] = exp
...
for n in incorrect_static_values:
    exp = load_batch_experiment(f'qwen_32B_incorrect_answer_{n}_')
    if exp:
...
incorrect_median = {}
for mult in [1, 2, 5]:
    exp = load_batch_experiment(f'qwen_32B_incorrect_dynamic_median_{mult}x_')
    if exp:
...
incorrect_max = {}
for mult in [1, 2, 5]:
    exp = load_batch_experiment(f'qwen_32B_incorrect_dynamic_max_{mult}x_')
    if exp:
...
ax_table.axis('off')
ax_table.set_title('BBQ Experiment Summary - Qwen-32B', fontsize=14, fontweight='bold', pad=20)
...
if SAVE_FIGURES:
    save_path = os.path.join(OUTPUT_DIR, 'qwen_32B_summary_table.png')
...
fig_detailed = plot_detailed_table(
    ...
    title="BBQ Detailed Category Breakdown - Qwen-32B",
    save=SAVE_FIGURES,
    filename='qwen_32B_detailed_table.png'
)
...
def plot_accuracy_grid(accuracy_type='sample', save=False):
    ...
    fig.suptitle(f'BBQ {accuracy_type.title()} Accuracy vs. Token/Insertion Count - Qwen-32B', 
    ...
    if save:
        filename = f'qwen_32B_{accuracy_type}_accuracy.png'
        save_path = os.path.join(OUTPUT_DIR, filename)
        plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='white')
        print(f"✓ Saved: {save_path}")
