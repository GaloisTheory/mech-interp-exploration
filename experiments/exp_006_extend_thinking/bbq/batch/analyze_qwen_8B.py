#!/usr/bin/env python3
"""BBQ Experiment Analysis for Qwen-8B
...
ax_table.axis('off')
ax_table.set_title('BBQ Experiment Summary - Qwen-8B', fontsize=14, fontweight='bold', pad=20)
...
fig_detailed = plot_detailed_table(
    ...
    title="BBQ Detailed Category Breakdown - Qwen-8B",
    save=SAVE_FIGURES,
    filename='qwen_8B_detailed_table.png'
)
...
def plot_accuracy_grid(accuracy_type='sample', save=False):
    ...
    fig.suptitle(f'BBQ {accuracy_type.title()} Accuracy vs. Token/Insertion Count - Qwen-8B', 
    ...
    if save:
        filename = f'qwen_8B_{accuracy_type}_accuracy.png'
        save_path = os.path.join(OUTPUT_DIR, filename)
        plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='white')
        print(f"✓ Saved: {save_path}")
